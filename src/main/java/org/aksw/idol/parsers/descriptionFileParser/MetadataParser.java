 ...
 * 
 */
package org.aksw.idol.parsers.descriptionFileParser;

import java.util.HashMap;

import org.aksw.idol.mongodb.collections.DatasetDB;
import org.aksw.idol.mongodb.collections.DistributionDB;
import org.aksw.idol.services.mongodb.DatasetServices;
import org.aksw.idol.services.mongodb.DistributionServices;
...
public abstract class MetadataParser {
	...
	/**
	 * Add a new dataset in the list. The method will save it and return an
	 * respective ID
	 * 
	 * @param uri
	 *            the dataset uri
	 * @param isVocab
	 *            represents if the dataset is a vocabulary or a ontology
	 * @param title
	 *            the dataset title
	 * @param labelthe
	 *            dataset label
	 * @param provenance
	 *            whe the dataset came from
	 * @return the dataset
	 */
	 ...
	 * 
	 * @param uri
	 *            the distribution URI
	 * @param isVocab
	 *            represents if the dataset is a vocabulary or a ontology
	 * @param title
	 *            the distriution title
	 * @param format
	 *            the distribution format
	 * @param downloadURL
	 *            the distribution downloadURL
	 * @param topDataset
	 *            the top dataset
	 * @param topDatasetTitle
	 *            the top dataset title
	 * @param datasource
	 *            the datasource which holds the distribution
	 * @param repository
	 *            the repository which holds the distribution
	...
	public DistributionDB addDistribution(String uri, boolean isVocab, String title, String format, String downloadURL,
		...
		DistributionDB d = distributionServices.saveDistribution(uri, isVocab, title, format, downloadURL, topDataset,
				topDatasetTitle, datasource, repository, sparqlGraph, sparqlEndpoint);
		addDistribution(d);
		return d;
	}
