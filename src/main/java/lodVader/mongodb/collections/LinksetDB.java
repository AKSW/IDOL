...
public class LinksetDB extends DBSuperClass {
	...
	public void removeAllLinks(int distributionID) {
		...
		or.add(query);
		or.add(query2);
		DBSuperClass.getDBInstance().getCollection(LinksetDB.COLLECTION_NAME).remove(new BasicDBObject("$or", or));
		...
		DBSuperClass.getDBInstance().getCollection(TopValidLinks.COLLECTION_NAME).remove(new BasicDBObject("$or", or));
