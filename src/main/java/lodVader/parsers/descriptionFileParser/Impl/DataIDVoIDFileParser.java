...
/**
 * @author ciro parser for DataID, VoID and DCAT files
 */
public class DataIDVoIDFileParser {
//	public class DataIDVoIDFileParser implements DescriptionFileParserInterface {
...
//			try {
//				iterateSubsetsNew(datasetsStmt, "", "", null, true);
//			} catch (LODVaderLODGeneralException | LODVaderFormatNotAcceptedException e) {
...
//
//		// find primaryTopic
//		datasetsStmt = inModel.listStatements(null, RDFProperties.primaryTopic, (RDFNode) null);
//
//		Resource topic = null;
//
//		if (datasetsStmt.hasNext())
...
//			if (topic == null)
//				datasetsStmt = inModel.listStatements(null, RDFProperties.type, datasetResource);
//			else
//				datasetsStmt = inModel.listStatements(topic, RDFProperties.type, datasetResource);
//			if (datasetsStmt.hasNext()) {
...
//	 */
//	private void iterateSubsetsNew(StmtIterator stmtDatasets, String parentDataset, String topDatasetID,
//			String topDatasetTitle, boolean isTopDataset)
//			throws LODVaderLODGeneralException, LODVaderFormatNotAcceptedException {
//
...
//			datasetMongoDBObj.setUri(datasetURI);
//
...
//				datasetMongoDBObj
//						.setTitle(dataset.getSubject().getProperty(RDFProperties.title).getObject().toString());
//			} else
...
//				datasetMongoDBObj
//						.setLabel(dataset.getSubject().getProperty(RDFProperties.label).getObject().toString());
//			} else
//				datasetMongoDBObj.setLabel(datasetURI);
//
//			// setting TOP dataset
//			if (isTopDataset) {
//				topDatasetID = datasetMongoDBObj.getID();
...
//
//			datasetMongoDBObj.setDescriptionFileParser(repositoryAddress);
//
//			try {
...
//			StmtIterator stmtDatasets2 = inModel.listStatements(dataset.getSubject().asResource(), RDFProperties.subset,
//					(RDFNode) null);
//
...
//				stmtLinkset = inModel.listStatements(subset.getObject().asResource(), RDFProperties.type,
...
//						stmtDatasets3 = inModel.listStatements(subset.getObject().asResource(), RDFProperties.type,
//								datasetsp);
...
//
//					if (stmtDatasets3.hasNext()) {
//						iterateSubsetsNew(stmtDatasets3, datasetMongoDBObj.getLODVaderID(), topDatasetID,
//								topDatasetTitle, false);
//						// datasetMongoDBObj.addSubsetID(subset.getObject()
//						// .toString());
//						
//						datasetMongoDBObj.addSubsetID(new DatasetDB(subset.getObject().toString()).getLODVaderID());
//						try {
//							datasetMongoDBObj.update(true);
//						} catch (LODVaderMissingPropertiesException | LODVaderObjectAlreadyExistsException
//								| LODVaderNoPKFoundException e) {
...
//				stmtDistribution = inModel.listStatements(dataset.getSubject().asResource(), distributionProperty,
//						(RDFNode) null);
//
//				// special treatment for VOID file
//				if (stmtDistribution.hasNext()
//						&& distributionProperty.equals(ResourceFactory.createProperty(NS.VOID_URI, "dataDump"))) {
//					Statement stmtDistribution2 = stmtDistribution.next();
//					try {
//						addDistribution(stmtDistribution2, stmtDistribution2, datasetMongoDBObj, topDatasetTitle,
//								topDatasetID);
...
//						if (!stmtDistribution.hasNext() || distributionStmt.getObject().toString().contains(".nt")) {
//							// find downloadURL property
//							StmtIterator stmtDownloadURL = null;
//
//							for (Property downloadProperty : RDFProperties.downloadURL) {
//								stmtDownloadURL = inModel.listStatements(distributionStmt.getObject().asResource(),
...
//
//								try {
//									if (FileUtils.acceptedFormats(downloadURLStmt.getObject().toString())) {
//
//										downloadURLFound = true;
//										addDistribution(downloadURLStmt, distributionStmt, datasetMongoDBObj,
//												topDatasetTitle, topDatasetID);
//
//									}
//								} catch (Exception ex) {
//									ex.printStackTrace();
...
//
//								downloadURLFound = true;
//								addDistribution(downloadURLStmt, distributionStmt, datasetMongoDBObj, topDatasetTitle,
//										topDatasetID);
//
//							}
...
//	}
//
//	public void addDistribution(Statement downloadURLStmt, Statement stmtDistribution, DatasetDB subsetMongoDBObj,
//			String topDatasetTitle, int topDatasetID) throws MalformedURLException {
//
//		logger.info("Distribution found: downloadURL: " + downloadURLStmt.getObject().toString());
//
...
//		DistributionDB distributionMongoDBObj = new DistributionDB(downloadURLStmt.getObject().toString());
//
//		// do not overlap LOV datasets
//		if (distributionMongoDBObj.getIsVocabulary())
//			return;
//
//		distributionMongoDBObj.setResourceUri(stmtDistribution.getSubject().toString());
//
...
//
//		distributionMongoDBObj.setDownloadUrl(downloadURLStmt.getObject().toString());
//
//		// case there is title property
//		try {
//			if (stmtDistribution.getSubject().getProperty(RDFProperties.title) != null) {
//				distributionMongoDBObj
//						.setTitle(stmtDistribution.getProperty(RDFProperties.title).getObject().toString());
//
//			}
//		} catch (Exception e) {
//			if (stmtDistribution.getSubject().getProperty(RDFProperties.title) != null) {
//				distributionMongoDBObj.setTitle(
//						stmtDistribution.getSubject().getProperty(RDFProperties.title).getObject().toString());
//
//			}
//		}
//
...
//			distributionMongoDBObj.setFormat(FormatsUtils.getEquivalentFormat(
//					stmtDistribution.getSubject().getProperty(RDFProperties.format).getObject().toString()));
//		}
...
//
//			try {
//				if (stmtDistribution.getObject().asResource().getProperty(RDFProperties.format).getObject().asResource()
//						.getProperty(RDFProperties.rdfValue).getObject() != null) {
//					distributionMongoDBObj.setFormat(FormatsUtils.getEquivalentFormat(
//							stmtDistribution.getObject().asResource().getProperty(RDFProperties.format).getObject()
//									.asResource().getProperty(RDFProperties.rdfValue).getObject().toString()));
//
//				}
//			} catch (Exception e) {
//
//				// else
//				distributionMongoDBObj.setFormat(FormatsUtils.getEquivalentFormat(stmtDistribution.getObject().asResource()
...
//			distributionMongoDBObj.setStatus(DistributionStatus.WAITING_TO_STREAM);
//		}
//		try {
...
//		for (Resource datasetResource : RDFProperties.Dataset) {
//			hasSomeDataset = inModel.listResourcesWithProperty(RDFProperties.type, datasetResource);
...
//			inModel.write(new FileOutputStream(new File(LODVaderProperties.FILE_URL_PATH + fileURLHash)));
//		} else {
...
//		if (format.equals(FormatsUtils.DEFAULT_NTRIPLES))
//			return "N-TRIPLES";
//		else if (format.equals(FormatsUtils.DEFAULT_TURTLE))
//			return "TTL";
